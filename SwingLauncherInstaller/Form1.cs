...
namespace SwingLauncherInstaller
{
	public partial class Form1 : Form
	{
		...
		private async void button2_Click(object sender, EventArgs e)
		{
			...
			{
				if(MessageBox.Show("A folder with that name already exists, do you want the folder to be overwritten?", "Overwrite folder?", MessageBoxButtons.YesNo) == DialogResult.Yes)
				{
					await Task.Factory.StartNew(() =>
					{
						...
						startInfo.WorkingDirectory = textBox1.Text + @"\SwingLauncher";
						startInfo.Arguments = "/C echo a&cd /d " + textBox1.Text + @"\SwingLauncher&rmdir /Q /S " + textBox1.Text + @"\SwingLauncher";
			...
			{
				MessageBox.Show("Fail", "Could not download version!", MessageBoxButtons.OK, MessageBoxIcon.Error);
				return;
			}
